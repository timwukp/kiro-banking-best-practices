...
echo "== apply lockdown =="
bash "$MDM/lockdown-linux.sh" >/dev/null 2>&1 && ok "lockdown-linux.sh ran" || ng "lockdown-linux.sh failed"
...
bash "$MDM/lockdown-linux.sh" >/dev/null 2>&1
...
ev '"rm -rf /"'       | bash "$HOOKS/destructive-fs-guard.sh" 2>/dev/null && ng "should block rm -rf /"       || ok "blocks rm -rf /"
ev '"rm -rf .git"'    | bash "$HOOKS/destructive-fs-guard.sh" 2>/dev/null && ng "should block rm -rf .git"    || ok "blocks rm -rf .git"
ev '"find . -delete"' | bash "$HOOKS/destructive-fs-guard.sh" 2>/dev/null && ng "should block find -delete"   || ok "blocks find -delete"
ev '"rm -rf ./build"' | bash "$HOOKS/destructive-fs-guard.sh" 2>/dev/null && ok "allows scoped rm -rf ./build" || ng "false positive on ./build"
